...
io.on('connection', (socket) => {
    ...
    socket.on('register', (data, callback) => {
        ...
        db.get(`SELECT * FROM users WHERE telegram_id = ?`, [telegram_id], (err, row) => {
            if (!row) {
                db.run(`INSERT INTO users (telegram_id, name, avatar, stars) VALUES (?, ?, ?, 1000)`, [telegram_id, name, avatar], function(err) {
                    if (callback) callback({ success: true, stars: 1000, name, telegram_id, turnover: 0, games_played: 0, wins: 0 });
                });
            } else {
                if (callback) callback({ success: true, stars: row.stars, name: row.name, telegram_id, turnover: row.turnover || 0, games_played: row.games_played || 0, wins: row.wins || 0 });
            }
        });
    });
